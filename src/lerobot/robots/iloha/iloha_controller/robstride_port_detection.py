...
async def detect_robstride_ports() -> RobStridePorts:
    """RobStrideの左右ポートをモータIDへの読取り応答から自動検出する。"""
    ...
    matches = {
        side: [port for port, detected_side in zip(candidates, classifications, strict=True) if detected_side == side]
    ...
    if len(matches["left"]) != 1 or len(matches["right"]) != 1:
        ...
        raise RuntimeError(
            ...
            f" expected left IDs={LEFT_ROBSTRIDE_IDS}, right IDs={RIGHT_ROBSTRIDE_IDS}, results={results}"
        )
